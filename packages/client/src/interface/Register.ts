...
export const UserFirstNameModel = z.string()
...
export const UserPasswordModel = z.string()
  .min(MIN_PASSWORD, `Пароль не может быть короче ${MIN_PASSWORD} символов`)
  .regex(PASSWORD_REGEX, `Пароль может содержать от 8 до 40 символов,
     обязательно хотя бы одна заглавная буква и цифра`);

export const UserEmailModel = z.string().email('Введите корректный Email');

export const UserPhoneModel = z.string()
